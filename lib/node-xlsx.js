...
var nodexlsx = function() {

}

nodexlsx.prototype.parse = function(mixed) {
  if(typeof mixed === 'string') mixed = fs.readFileSync(mixed);
  return this.xlsx(mixed.toString('base64'));
}

nodexlsx.prototype.build = function(object, callback) {
  var xlsdata = this.xlsx(object); // [ 'base64', 'zipTime', 'processTime', 'href' ]
  var data = [];
  xlsdata.on('data', function(chunk) {
      data.push(chunk);
  }).on('end', function() {
  ...
  xlsdata.finalize(function(err, written) {
    if (err) {
      callback(err);
    }
...
nodexlsx.prototype.buildStream = function(object, stream, callback) {
  var fs = require('fs');

  var data = this.xlsx(object); // [ 'base64', 'zipTime', 'processTime', 'href' ]
  data.pipe(stream);
  data.finalize(function(err, written) {
    if (err) {
      callback(err);
    }
...
nodexlsx.prototype.xlsx = function(file) {
    var archiver = require('archiver');
    var result, zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, style,
    ...
    if (typeof file === 'string') { // Load
      ...
      while (w--) { // Generate worksheet (gather sharedStrings), and possibly table files, then generate entries for constant files below
        ...
        while (++i < l) {
          ...
          while (++j < k) {
            ...
            if (val && typeof val === 'string' && (!isFinite(val) || cell.formatCode == '@')) { // If value is string, and not string of just a number, place a sharedString reference instead of the value
              val = escapeXML(val);
        ...
        zip.append(s + '</worksheet>', {name: 'xl/worksheets/sheet' + id + '.xml'});
        //}
        ...
        if (worksheet.table) {
          ...
          zip.append(s, {name: 'xl/tables/table' + id + '.xml'});
          zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table' + id + '.xml"/></Relationships>',
            {name: 'xl/worksheetss/_rels/sheet' + id + '.xml.rels'});
      ...
      zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
        ...
        + '<x14:slicerStyles defaultSlicerStyle="SlicerStyleLight1"/></ext></extLst></styleSheet>', {name: 'xl/styles.xml'});
      //}
      //{ [Content_Types].xml
      ...
      //}
      //{ docProps/app.xml
      zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>XLSX.js</Application><DocSecurity>0</DocSecurity><ScaleCrop>false</ScaleCrop><HeadingPairs><vt:vector size="2" baseType="variant"><vt:variant><vt:lpstr>Worksheets</vt:lpstr></vt:variant><vt:variant><vt:i4>'
      ...
      zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
      ...
      zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="'
        + sharedStrings[1] + '" uniqueCount="' + sharedStrings[0].length + '"><si><t>' + sharedStrings[0].join('</t></si><si><t>') + '</t></si></sst>',
        {name: 'xl/sharedStrings.xml'});
      //}
      //{ xl/workbook.xml
      zip.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">'
        ...
        + worksheets.join('') + '</sheets><calcPr calcId="145621"/></workbook>', {name: 'xl/workbook.xml'});
      //}
      //}
